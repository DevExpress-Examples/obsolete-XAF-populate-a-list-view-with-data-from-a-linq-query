...
namespace Dennis.Linq {
    public static class LinqCollectionSourceHelper {
        public static void CreateCustomCollectionSource(object sender, CreateCustomCollectionSourceEventArgs e) {
            DictionaryNode listViewInfo = ((XafApplication)sender).FindViewInfo(e.ListViewID);
            if (listViewInfo == null) return;
            string queryMethod = listViewInfo.GetAttributeValue("XPQueryMethod");
            if (string.IsNullOrEmpty(queryMethod)) return;
            IQueryable query = LinqCollectionSourceHelper.InvokeMethod(e.ObjectType, queryMethod, e.ObjectSpace.Session);
